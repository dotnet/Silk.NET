...
namespace Silk.NET.Maths
{
    ...
    public readonly struct Half : IComparable, IFormattable, IComparable<Half>, IEquatable<Half>
    {
        ...
        {
            get { return (sbyte) ((m_value & ExponentMask) >> ExponentShift); }
        }

        private ushort Significand
        {
            get { return (ushort) ((m_value & SignificandMask) >> SignificandShift); }
        }
        ...
        {
            int shiftDist = Helpers.LeadingZeroCount(value) - 21;
            if (shiftDist >= 0)
            {
                return value != 0
            ...
            shiftDist += 4;
            uint sig = shiftDist < 0 ? Helpers.ShiftRightJam(value, -shiftDist) : value << shiftDist;
        ...
        {
            int shiftDist = Helpers.LeadingZeroCount(value) - 53;
        ...
        public static explicit operator Half(float value)
        {
            ...
            uint floatInt = Helpers.ToUInt32(value);
            bool sign = (floatInt & Helpers.SingleSignMask) >> Helpers.SingleSignShift != 0;
            int exp = (int) (floatInt & Helpers.SingleExponentMask) >> Helpers.SingleExponentShift;
            uint sig = floatInt & Helpers.SingleSignificandMask;
        ...
        public static explicit operator Half(double value)
        {
            ...
            ulong doubleInt = Helpers.ToUInt64(value);
            bool sign = (doubleInt & Helpers.DoubleSignMask) >> Helpers.DoubleSignShift != 0;
            int exp = (int) ((doubleInt & Helpers.DoubleExponentMask) >> Helpers.DoubleExponentShift);
            ulong sig = doubleInt & Helpers.DoubleSignificandMask;
            ...
            uint sigHalf = (uint) Helpers.ShiftRightJam(sig, 38);
            if ((exp | (int) sigHalf) == 0)
            {
                return new Half(sign, 0, 0);
            }
        ...
        {
            int shiftDist = Helpers.LeadingZeroCount(sig) - 16 - 5; // No LZCNT for 16-bit
