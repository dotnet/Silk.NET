...
namespace Silk.NET.Maths
{
    ...
    {
        /// <summary>
        /// Performs hardware-accelerated NotEqual on 256-bit vectors.
        /// </summary>
        [MethodImpl(Scalar.MaxOpt)]
        public static Vector256<T> NotEqual<T>(Vector256<T> left, Vector256<T> right) where T : unmanaged
        {
            ...
            static Vector256<T> Single(Vector256<T> left, Vector256<T> right)
            {
                if (typeof(T) == typeof(float))
                {
                    ...
                    if (Avx2.IsSupported)
                    {
                        return Avx2.CompareNotEqual(left.AsSingle(), right.AsSingle()).As<float, T>();
                    }
#endif
                }
            ...
            static Vector256<T> Double(Vector256<T> left, Vector256<T> right)
            {
                if (typeof(T) == typeof(double))
                {
                    ...
                    if (Avx2.IsSupported)
                    {
                        return Avx2.CompareNotEqual(left.AsDouble(), right.AsDouble()).As<double, T>();
                    }
#endif
                }
