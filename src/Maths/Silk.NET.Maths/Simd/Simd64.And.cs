...
namespace Silk.NET.Maths
{
    ...
    {
        /// <summary>
        /// Performs hardware-accelerated And on 64-bit vectors.
        /// </summary>
        [MethodImpl(Scalar.MaxOpt)]
        public static Vector64<T> And<T>(Vector64<T> left, Vector64<T> right) where T : unmanaged
        {
            ...
            static Vector64<T> Other8byte(Vector64<T> left, Vector64<T> right)
            {
                if (sizeof(T) == 8)
                {
                    var res = Scalar.And(Unsafe.As<Vector64<T>, T>(ref left), Unsafe.As<Vector64<T>, T>(ref right));
