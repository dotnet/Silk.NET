...
namespace Silk.NET.Maths
{
    ...
    {
        /// <summary>
        /// Performs hardware-accelerated GreaterThan on 64-bit vectors.
        /// </summary>
        [MethodImpl(Scalar.MaxOpt)]
        public static Vector64<T> GreaterThan<T>(Vector64<T> left, Vector64<T> right) where T : unmanaged
        {
            ...
            static Vector64<T> Other8byte(Vector64<T> left, Vector64<T> right)
            {
                if (sizeof(T) == 8)
                {
                    var res = Scalar.GreaterThan(Unsafe.As<Vector64<T>, T>(ref left), Unsafe.As<Vector64<T>, T>(ref right));
            ...
            static Vector64<T> Other(Vector64<T> left, Vector64<T> right)
            {
                ...
                {
                    vec = vec.WithElement(i, Scalar.GreaterThan(left.GetElement(i), right.GetElement(i)) ? Scalar<T>.AllBitsSet : Scalar<T>.Zero);
                }
                return vec;
            }
        }
