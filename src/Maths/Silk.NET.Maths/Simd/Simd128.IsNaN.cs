...
namespace Silk.NET.Maths
{
    ...
    {
        /// <summary>
        /// Performs hardware-accelerated IsNaN on 128-bit vectors.
        /// </summary>
        [MethodImpl(Scalar.MaxOpt)]
        public static Vector128<T> IsNaN<T>(Vector128<T> vector) where T : unmanaged
        {
            ...
            static Vector128<T> Other(Vector128<T> vector)
            {
                ...
                {
                    vec.WithElement(i, Scalar.IsNaN(vector.GetElement(i)) ? Scalar<T>.AllBitsSet : Scalar<T>.Zero);
                }
                return vec;
            }
        }
