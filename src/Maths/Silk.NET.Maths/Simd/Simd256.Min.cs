...
namespace Silk.NET.Maths
{
    ...
    {
        /// <summary>
        /// Performs hardware-accelerated Min on 256-bit vectors.
        /// </summary>
        [MethodImpl(Scalar.MaxOpt)]
        public static Vector256<T> Min<T>(Vector256<T> left, Vector256<T> right) where T : unmanaged
        {
            ...
            static Vector256<T> Single(Vector256<T> left, Vector256<T> right)
            {
                if (typeof(T) == typeof(float))
                {
                    ...
                    if (Avx2.IsSupported)
                    {
                        return Avx2.Min(left.AsSingle(), right.AsSingle()).As<float, T>();
                    }
#endif
                }
            ...
            static Vector256<T> Double(Vector256<T> left, Vector256<T> right)
            {
                if (typeof(T) == typeof(double))
                {
                    ...
                    if (Avx2.IsSupported)
                    {
                        return Avx2.Min(left.AsDouble(), right.AsDouble()).As<double, T>();
                    }
#endif
                }
