...
namespace Silk.NET.Maths
{
    public readonly struct Box3<T> : IEquatable<Box3<T>>, IFormattable where T : unmanaged, IFormattable
    {
        ...
        public Box3(T minX, T minY, T minZ, T maxX, T maxY, T maxZ)
            : this(new Vector3<T>(minX, minY, minZ), new Vector3<T>(maxX, maxY, maxZ)) { }
        ...
        public bool Contains(Vector3<T> point, bool boundaryInclusive = false)
            =>
                boundaryInclusive
                    ...
                    : Larger(point.X, Min.X) && Larger(point.Y, Min.Y) && Larger(point.Z, Min.Z) &&
                      Smaller(point.X, Max.X) && Smaller(point.Y, Min.Y) && Smaller(point.Z, Min.Z);
            ...
            =>
                boundaryInclusive
                    ? LargerEquals(other.Min.X, Min.X) && LargerEquals(other.Min.Y, Min.Y) &&
                      LargerEquals(other.Min.Z, Min.Z) && SmallerEquals(other.Max.X, Max.X) &&
                      SmallerEquals(other.Max.Y, Min.Y) && SmallerEquals(other.Max.Z, Min.Z)
                    : Larger(other.Min.X, Min.X) && Larger(other.Min.Y, Min.Y) && Smaller(other.Max.X, Max.X) &&
                      Smaller(other.Max.Y, Min.Y) && Smaller(other.Max.Z, Min.Z);
