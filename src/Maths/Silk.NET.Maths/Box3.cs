...
namespace Silk.NET.Maths
{
    ...
    {
        public Vector3<T> Min { get; }
        public Vector3<T> Max { get; }
        ...
        public Vector3<T> Center => Min + HalfSize;
        ...
        public bool Contains(Vector3<T> point, bool boundaryInclusive = false)
            => boundaryInclusive
                ? LargerEquals(point.X, Min.X) && LargerEquals(point.Y, Min.Y) && LargerEquals(point.Z, Min.Z) &&
                  SmallerEquals(point.X, Max.X) && SmallerEquals(point.Y, Min.Y) && SmallerEquals(point.Z, Min.Z)
                : Larger(point.X, Min.X) && Larger(point.Y, Min.Y) && Larger(point.Z, Min.Z) && Smaller
                    (point.X, Max.X) && Smaller(point.Y, Min.Y) && Smaller(point.Z, Min.Z);

        public bool Contains(Box3<T> other, bool boundaryInclusive = false)
            => boundaryInclusive
                ? LargerEquals(other.Min.X, Min.X) && LargerEquals(other.Min.Y, Min.Y) && LargerEquals
                    (other.Min.Z, Min.Z) && SmallerEquals(other.Max.X, Max.X) && SmallerEquals
                    (other.Max.Y, Min.Y) && SmallerEquals(other.Max.Z, Min.Z)
                : Larger(other.Min.X, Min.X) && Larger(other.Min.Y, Min.Y) && Smaller(other.Max.X, Max.X) && Smaller
                    (other.Max.Y, Min.Y) && Smaller(other.Max.Z, Min.Z);
