module Silk.NET.Maths.Tests.ScalarTests.UInt32
...
let t = testList "Scalar UInt32 Tests" [
    ...
    testProperty "As" <|
        fun (a:float) -> Scalar.As<uint32>(a) = uint32 a
    ...
    testProperty "IsNormal" <| fun (a:uint32) -> Expect.equal (Scalar.IsNormal<uint32>(a)) true
]