module Silk.NET.Maths.Tests.ScalarTests.UInt16
...
let t = testList "Scalar UInt16 Tests" [
    testList "Generic Helper" [
        ...
        testCase "HalfPi" <| fun () -> Expect.throwsT<NotSupportedException> (fun () -> Scalar<uint16>.HalfPi |> ignore) "Scalar<uint16>.HalfPi throws"
    ...
    testProperty "As" <|
        fun (a:float) -> Scalar.As<uint16>(a) = uint16 a
    ...
    testList "Throw Helpers" [
        ...
        testCase "ThrowForNonFloatingPointType" <| fun () -> Expect.throwsT<NotSupportedException> (fun () -> Scalar.ThrowForNonFloatingPointType<uint16>()) "ThrowForNonFloatingPointType<uint16> throws NotSupportedException"
    ...
    testProperty "IsNormal" <| fun (a:uint16) -> Expect.equal (Scalar.IsNormal<uint16>(a)) true
]