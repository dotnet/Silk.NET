module Silk.NET.Maths.Tests.ScalarTests.UInt64
...
let t = testList "Scalar UInt64 Tests" [
    testList "Generic Helper" [
        ...
        testCase "HalfPi" <| fun () -> Expect.throwsT<NotSupportedException> (fun () -> Scalar<uint64>.HalfPi |> ignore) "Scalar<uint64>.HalfPi throws"
    ...
    testProperty "IsNormal" <| fun (a:uint64) -> Expect.equal (Scalar.IsNormal<uint64>(a)) true
]