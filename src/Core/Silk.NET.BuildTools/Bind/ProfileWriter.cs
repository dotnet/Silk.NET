...
namespace Silk.NET.BuildTools.Bind
{
    ...
    {
        public const string NativeImpl =
        ...
        public static void WriteMixedModeClasses(this Project project, Profile profile, string folder)
        {
            ...
            if (project.IsRoot)
            {
                ...
                sw.WriteLine("{");
                sw.WriteLine
                    ($"    public unsafe partial class {profile.ClassName} : NativeAPI");
                sw.WriteLine("    {");
                ...
                foreach (var function in allFunctions)
                {
                    ...
                    sw.WriteLine($"        {NativeImpl}");
                    using (var sr = new StringReader(function.ToString()[..^1]))
                    {
                        string line;
                        var flPrefix = "public ";
                        while ((line = sr.ReadLine()) != null)
                    ...
                    sw.WriteLine($"            => Impl{function.ToString(null, true)}");
                    sw.WriteLine();
                }
                ...
                sw.WriteLine();
                sw.WriteLine($"        public {profile.ClassName}(INativeContext ctx)");
                sw.WriteLine("            : base(ctx)");
                sw.WriteLine("        {");
                sw.WriteLine("            InitializeNative();");
                sw.WriteLine("        }");
                sw.WriteLine("    }");
                sw.WriteLine("}");
                sw.WriteLine();
                ...
                if (!File.Exists(Path.Combine(folder, $"{profile.ClassName}.cs")))
                {
                    ...
                    (
                        ...
                        $"\n                 ? InstantiateExtension<T, {profile.ClassName}>(Context)" +
                        "\n                 : null"
                    );
            ...
            else
            {
                foreach (var (key, i) in project.Interfaces)
                {
                    ...
                    sw.WriteLine
                    (
                        $"    public unsafe partial class {name} : NativeExtension<{profile.ClassName}>"
                    );
                    sw.WriteLine("    {");
