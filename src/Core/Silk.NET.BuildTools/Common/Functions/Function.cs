...
namespace Silk.NET.BuildTools.Common.Functions
{
    ...
    public class Function : IEquatable<Function>
    {
        ...
        public string ToString(bool? @unsafe, bool call = false)
        {
            var sb = new StringBuilder();

            GetDeclarationString(sb, @unsafe, call);

            sb.Append("(");
            if (Parameters.Count > 0)
            {
                var parameterDeclarations = Parameters.Select(x => GetDeclarationString(x, call)).ToList();
        ...
        private void GetDeclarationString(StringBuilder sb, bool? @unsafe, bool call = false)
        {
            if (!call)
            {
        ...
        [NotNull]
        private static string GetDeclarationString([NotNull] Parameter parameter, bool call = false)
        {
            var sb = new StringBuilder();
            ...
            if (!call)
            {
                ...
                attributes.AddRange
                    (parameter.Attributes.Select(x => x.Name + "(" + string.Join(", ", x.Arguments) + ")"));
