...
namespace Silk.NET.OpenXR.Extensions.META
{
    ...
    public unsafe partial class MetaPerformanceMetrics : NativeExtension<XR>
    {
        ...
        public unsafe partial Result SetPerformanceMetricsStateMeta([Count(Count = 0)] Session session, [Count(Count = 0), Flow(FlowDirection.In)] PerformanceMetricsStateMETA* state);

        /// <summary>To be documented.</summary>
        [NativeApi(EntryPoint = "xrSetPerformanceMetricsStateMETA", Convention = CallingConvention.Winapi)]
        public partial Result SetPerformanceMetricsStateMeta([Count(Count = 0)] Session session, [Count(Count = 0), Flow(FlowDirection.In)] in PerformanceMetricsStateMETA state);
