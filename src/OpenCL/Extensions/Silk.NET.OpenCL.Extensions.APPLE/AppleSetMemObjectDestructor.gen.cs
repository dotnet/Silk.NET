...
namespace Silk.NET.OpenCL.Extensions.APPLE
{
    ...
    public unsafe partial class AppleSetMemObjectDestructor : NativeExtension<CL>
    {
        ...
        public unsafe int SetMemObjectDestructor([Flow(FlowDirection.In)] IntPtr memobj, [Flow(FlowDirection.In), PinObjectAttribute(PinMode.UntilNextCall)] NotifyCallback pfn_notify, [Flow(FlowDirection.Out)] void* user_data)
            => ImplSetMemObjectDestructor(memobj, pfn_notify, user_data);

        [NativeApi(EntryPoint = "clSetMemObjectDestructorAPPLE")]
        [System.Runtime.CompilerServices.MethodImpl((System.Runtime.CompilerServices.MethodImplOptions)(512 | 256))]
        public int SetMemObjectDestructor<T0>([Flow(FlowDirection.In)] IntPtr memobj, [Flow(FlowDirection.In), PinObjectAttribute(PinMode.UntilNextCall)] NotifyCallback pfn_notify, [Flow(FlowDirection.Out)] Span<T0> user_data) where T0 : unmanaged
            => ImplSetMemObjectDestructor<T0>(memobj, pfn_notify, user_data);
