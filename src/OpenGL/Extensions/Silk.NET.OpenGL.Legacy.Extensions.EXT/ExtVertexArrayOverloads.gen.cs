...
namespace Silk.NET.OpenGL.Legacy.Extensions.EXT
{
    ...
    {
        public static unsafe void ColorPointer<T0>(this ExtVertexArray thisApi, [Flow(FlowDirection.In)] int size, [Flow(FlowDirection.In)] ColorPointerType type, [Flow(FlowDirection.In)] uint stride, [Flow(FlowDirection.In)] uint count, [Count(Computed = "size, type, stride, count"), Flow(FlowDirection.In)] Span<T0> pointer) where T0 : struct
        {
            // SpanOverloader
            fixed (void* pointerSpp = pointer)
                thisApi.ColorPointer(size, type, stride, count, pointerSpp);
        }
