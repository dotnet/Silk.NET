...
public static unsafe partial class Windows
{
    ...
    public static extern BOOL SslEmptyCacheA(
        [NativeTypeName("LPSTR")] sbyte* pszTargetName,
        [NativeTypeName("DWORD")] uint dwFlags
    );

    [DllImport("schannel", ExactSpelling = true)]
    public static extern BOOL SslEmptyCacheW(
        [NativeTypeName("LPWSTR")] ushort* pszTargetName,
        [NativeTypeName("DWORD")] uint dwFlags
    );

    [DllImport("schannel", ExactSpelling = true)]
    public static extern void SslGenerateRandomBits(
        [NativeTypeName("PUCHAR")] byte* pRandomData,
        [NativeTypeName("LONG")] int cRandomData
    );

    [DllImport("schannel", ExactSpelling = true)]
    public static extern BOOL SslCrackCertificate(
        [NativeTypeName("PUCHAR")] byte* pbCertificate,
        [NativeTypeName("DWORD")] uint cbCertificate,
        [NativeTypeName("DWORD")] uint dwFlags,
        [NativeTypeName("PX509Certificate *")] X509Certificate** ppCertificate
    );

    [DllImport("schannel", ExactSpelling = true)]
    public static extern void SslFreeCertificate(
        [NativeTypeName("PX509Certificate")] X509Certificate* pCertificate
    );
    ...
    public static extern int SslGetServerIdentity(
        [NativeTypeName("PBYTE")] byte* ClientHello,
        [NativeTypeName("DWORD")] uint ClientHelloSize,
        [NativeTypeName("PBYTE *")] byte** ServerIdentity,
        [NativeTypeName("PDWORD")] uint* ServerIdentitySize,
        [NativeTypeName("DWORD")] uint Flags
    );

    [DllImport("schannel", ExactSpelling = true)]
    [return: NativeTypeName("SECURITY_STATUS")]
    public static extern int SslGetExtensions(
        [NativeTypeName("const BYTE *")] byte* clientHello,
        [NativeTypeName("DWORD")] uint clientHelloByteSize,
        SCH_EXTENSION_DATA* genericExtensions,
        byte genericExtensionsCount,
        [NativeTypeName("DWORD *")] uint* bytesToRead,
    ...
    public static extern int SslDeserializeCertificateStore(
        [NativeTypeName("CERT_BLOB")] CRYPT_DATA_BLOB SerializedCertificateStore,
    ...
    public static ReadOnlySpan<byte> UNISP_NAME_A =>
        "Microsoft Unified Security Protocol Provider"u8;
