...
public struct CD3DX12_PIPELINE_STATE_STREAM_RENDER_TARGET_FORMATS
{
    ...
    public CD3DX12_PIPELINE_STATE_STREAM_RENDER_TARGET_FORMATS(
        [NativeTypeName("D3D12_RT_FORMAT_ARRAY const &")] in D3D12_RT_FORMAT_ARRAY i
    )
    {
        pssType = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_RENDER_TARGET_FORMATS;
        pssInner = i;
    ...
    public static implicit operator CD3DX12_PIPELINE_STATE_STREAM_RENDER_TARGET_FORMATS(
        in D3D12_RT_FORMAT_ARRAY value
    ) => new CD3DX12_PIPELINE_STATE_STREAM_RENDER_TARGET_FORMATS(value);

    public static implicit operator D3D12_RT_FORMAT_ARRAY(
        in CD3DX12_PIPELINE_STATE_STREAM_RENDER_TARGET_FORMATS value
    ) => value.pssInner;
