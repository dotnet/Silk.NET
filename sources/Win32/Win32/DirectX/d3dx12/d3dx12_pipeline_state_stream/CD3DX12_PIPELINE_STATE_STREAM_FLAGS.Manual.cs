...
public struct CD3DX12_PIPELINE_STATE_STREAM_FLAGS
{
    ...
    public CD3DX12_PIPELINE_STATE_STREAM_FLAGS(
        [NativeTypeName("D3D12_PIPELINE_STATE_FLAGS const &")] in D3D12_PIPELINE_STATE_FLAGS i
    )
    {
        pssType = D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_FLAGS;
        pssInner = i;
    ...
    public static implicit operator CD3DX12_PIPELINE_STATE_STREAM_FLAGS(
        in D3D12_PIPELINE_STATE_FLAGS value
    ) => new CD3DX12_PIPELINE_STATE_STREAM_FLAGS(value);

    public static implicit operator D3D12_PIPELINE_STATE_FLAGS(
        in CD3DX12_PIPELINE_STATE_STREAM_FLAGS value
    ) => value.pssInner;
