...
public class FileSystemCacheProvider(ILogger<FileSystemCacheProvider> logger) : ICacheProvider
{
    ...
    /// <inheritdoc />
    public async Task<(string Path, bool IsNew)?> GetDirectory(string cacheKey, CacheIntent intent, CacheFlags flags)
    ...
    async Task<(string Path, bool IsNew)?> CoreGetDirectory(string cacheKey, CacheIntent intent, bool isUnstaged,
        CacheFlags flags)
    {
        ...
        if (sema is not null && ((isUnstaged && !Directory.Exists(path)) || (!isUnstaged && !File.Exists(path))))
        {
            ...
            else
            {
                ...
                if (File.Exists(path) && (flags & CacheFlags.NoHostDirectory) == 0)
                {
                    File.Move(path, path + ".tmp");
                    {
                        ...
                        await Parallel.ForEachAsync(arch.Entries, async (entry, ct) => {
                            ...
                            var unixFileMode = (UnixFileMode)((entry.ExternalAttributes >> 16) & 511);
    ...
    /// <inheritdoc />
    public async Task CommitFile(string cacheKey, CacheIntent intent, CacheFlags flags, string filePath, Stream stream)
    {
        var path = GetCachePath(cacheKey, intent);
    ...
    public Task<IEnumerable<string>> GetFiles(string cacheKey, CacheIntent intent, CacheFlags flags)
    {
        ...
        return Task.FromResult<IEnumerable<string>>(Directory.GetFiles(path, "*", SearchOption.AllDirectories));
    }

    /// <inheritdoc />
    public Task<Stream> GetFile(string cacheKey, CacheIntent intent, CacheFlags flags, string filePath)
    {
        var path = GetCachePath(cacheKey, intent);
        if ((flags & CacheFlags.NoHostDirectory) != 0 && _noHostZips.TryGetValue(path, out var zip))
        {
