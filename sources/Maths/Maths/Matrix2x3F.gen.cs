namespace Silk.NET.Maths
{
    ...
    partial struct Matrix2x3F<T> : IEquatable<Matrix2x3F<T>> where T : IFloatingPointIeee754<T>
    {
        ...
        public static Matrix2x3F<T> operator +(Matrix2x3F<T> left, Matrix2x3F<T> right) => new(left.Row1 + right.Row1, left.Row2 + right.Row2);
        ...
        public static Matrix2x3F<T> operator *(Matrix2x2F<T> left, Matrix2x3F<T> right) => new(left.M11 * right.Row1 + left.M12 * right.Row2, left.M21 * right.Row1 + left.M22 * right.Row2);
        /// <summary>Multiplies a matrix by another matrix.</summary>
        /// <param name = "left">The first source matrix.</param>
        /// <param name = "right">The second source matrix.</param>
        /// <returns>The result of the multiplication.</returns>
        public static Matrix2x2F<T> operator *(Matrix2x3F<T> left, Matrix3x2F<T> right) => new(left.M11 * right.Row1 + left.M12 * right.Row2 + left.M13 * right.Row3, left.M21 * right.Row1 + left.M22 * right.Row2 + left.M23 * right.Row3);
    }
}