...
public readonly unsafe struct PfnFfxQuery : IDisposable
{
    private readonly void* Pointer;
    public delegate* unmanaged<void**, ApiHeader*, uint> Handle =>
        (delegate* unmanaged<void**, ApiHeader*, uint>)Pointer;

    public PfnFfxQuery(delegate* unmanaged<void**, ApiHeader*, uint> ptr) => Pointer = ptr;

    public PfnFfxQuery(PfnFfxQueryDelegate proc) => Pointer = SilkMarshal.DelegateToPtr(proc);

    public void Dispose() => SilkMarshal.Free(Pointer);

    public static implicit operator PfnFfxQuery(
