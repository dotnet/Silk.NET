...
public class Ref2DTests
{
    ...
    [Test]
    public void SingleStringPtrUtf8FromReadOnlySpan()
    {
        ReadOnlySpan<byte> thing = Encoding.UTF8.GetBytes(STR_1 + "\0");
        var thingPtr = thing.AsRef2D();
        Assert.That((string)thingPtr.Ref, Is.EqualTo(STR_1));
        Assert.That((string)thingPtr[0], Is.EqualTo(STR_1));
        Assert.That(thingPtr[0][0], Is.EqualTo(STR_1[0]));
    ...
    [Test]
    public unsafe void SingleStringPtrUtf16FromReadOnlySpan()
    {
        ReadOnlySpan<char> thing = MemoryMarshal.Cast<byte, char>(
            Encoding.Unicode.GetBytes(STR_1 + "\0")
        );
        var thingPtr = thing.AsRef2D();
        Assert.That((string)thingPtr.Ref, Is.EqualTo(STR_1));
        Assert.That((string)thingPtr[0], Is.EqualTo(STR_1));
    ...
    [Test]
    public void SingleStringPtrUtf32FromReadOnlySpan()
    {
        ReadOnlySpan<uint> thing = MemoryMarshal.Cast<byte, uint>(
            Encoding.UTF32.GetBytes(STR_1 + "\0")
        );
        var thingPtr = thing.AsRef2D();
        Assert.That((string)thingPtr.Ref, Is.EqualTo(STR_1));
        Assert.That((string)thingPtr[0], Is.EqualTo(STR_1));
