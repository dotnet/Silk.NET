...
public static unsafe partial class ID3D12VideoProcessCommandListTests
{
    ...
    {
        Assert.That(
            typeof(ID3D12VideoProcessCommandList).GUID,
            Is.EqualTo(IID_ID3D12VideoProcessCommandList)
        );
    }
    ...
    {
        Assert.That(
            Marshal.SizeOf<ID3D12VideoProcessCommandList>(),
            Is.EqualTo(sizeof(ID3D12VideoProcessCommandList))
        );
    }
