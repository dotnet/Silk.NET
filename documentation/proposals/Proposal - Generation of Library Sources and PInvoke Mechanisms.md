...
- An indexer **must** be present accepting a `nuint` for the index, returning a `ref T` or `ref readonly T` (depending on the outer dimension mutability) where `T` is either the generic type (or `byte` for non-generic variants), a "pointer-like" type of the inner dimension (i.e. a `PtrPtr<T>`'s indexer will return a `Ptr<T>`) of type being indexed.
